...
/** Every granted power carrying a choice_bonus_to_specific_skills effect (e.g. Esperteza Vulpina). The picks become {tag:'skill', op:'add'} custom_effects on that power at save time. */
export function resolveSkillBonusChoicePowers(grantedPowerIds: Set<number>, powers: Power[]): SkillBonusChoicePower[] {
  return powers
    ...
    .flatMap((power) => {
      const effect = (power.effects ?? []).find((e) => e.tag === 'choice_bonus_to_specific_skills');
