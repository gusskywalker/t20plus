 ...
 * a negative one, using the current Força (base plus active mod_str) (asymmetric penalty, per claude-stuff/rules/inventory-slots.md),
...
export function calculateMaxSlots(character: Character, powers: Power[]): number {
  const str = calculateStatBonus(character, 'str');
  const baseline = str >= 0 ? 10 + 2 * str : 10 + 1 * str;
  return baseline + resolveTag(resolveEffectSentinels(getActiveEffects(character), character, powers), 'mod_inventory_space');
}
