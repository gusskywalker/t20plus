...
- Margins use multiples of 20px (20, 40...) — 10, 12, 24 are used on VERY specific places, always by the user when styling something, never by Claude by default.
...
- A `//TODO` that got done is deleted outright — never rewritten into a new note about what's left.
...
- For new mechanics, weigh both "how does this show on the sheet/roll screen now" — but only for stuff that's cheap to generalize anyway (flat numeric/reusable tags). Don't force a future-engine shape onto a weird one-off power — model `usability`/`pm_cost`/`prerequisites`, skip `effects` on those.
...
- Seeding data (a power/spell row with existing tags) needs no go-ahead. Any new or changed logic (a new tag, a resolver/modal/calculator change, a schema field) is proposed first and only written after the user says go — even when the design felt agreed.
...
- `claude-stuff/known-to-test.md` (like `known-todos.md`) is the user's own file — never edit it.
